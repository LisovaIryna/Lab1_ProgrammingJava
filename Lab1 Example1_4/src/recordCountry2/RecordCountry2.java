...
public class RecordCountry2 {
    ...
    public static Strana[] setCountryArr(int k) {
        ...
        System.out.println("Введите информацию о станах: ");
        for (int i = 0; i < country.length; i++) {
            country[i] = new Strana();
            System.out.print("Название " + (i + 1) + "-й страны => "); //Украина--США--Индия--Австралия

            country[i].name = sc.nextLine();
            System.out.print("Площадь " + (i + 1) + "-й страны => "); // 0,6--9,8--3,2--7,7 млн кв. км (2010 г.)
    ...
    public static int NomMax(Strana[] st) {
        int nommax = 0;   // номер элемента страны с макс. площадью (начальное значение)
